...
12. OpenPanel for product analytics (self-hosted adapter boundary, started through the main repo-managed Compose entrypoint with assets owned by `ops/docker/analytics/`).
...
15. Kong and Vault through the main repo-managed Compose entrypoint with assets owned by `ops/docker/security/` for edge and secrets workflows.
