...
| --------------------------------- | --------------- | ------------------------------------ | ---------------------------------------------- |
| Bun                               | `1.3.11`        | `1.3.13`                             | Patch bump before UI work starts.              |
| React                             | `19.2.4`        | `19.2.6`                             | Patch bump.                                    |
| React DOM                         | `19.2.4`        | `19.2.6`                             | Patch bump with React.                         |
| `@tanstack/react-start`           | `1.167.16`      | `1.167.65`                           | Patch bump before major route work.            |
| `@tanstack/react-router`          | `1.168.10`      | `1.169.2`                            | Patch bump.                                    |
| `@tanstack/react-router-devtools` | `1.166.11`      | `1.166.13`                           | Patch bump if devtools remain enabled locally. |
| Vite                              | `8.0.8`         | `8.0.12`                             | Patch bump.                                    |
| `@vitejs/plugin-react`            | `6.0.1`         | `6.0.1`                              | Already current.                               |
| TypeScript                        | `6.0.2`         | `6.0.3`                              | Patch bump.                                    |
| Vitest                            | `4.0.7`         | `4.1.6`                              | Bump all Vitest packages together.             |
| `@vitest/browser-playwright`      | `4.0.7`         | `4.1.6`                              | Bump with Vitest.                              |
| `@vitest/coverage-v8`             | `4.0.7`         | `4.1.6`                              | Bump with Vitest.                              |
| Playwright                        | `1.59.1`        | `1.60.0`                             | Patch bump.                                    |
...
Before implementation begins, perform one focused dependency refresh for the app-facing stack and the broader repo-owned upgrade set recorded in `specs/01-platform/architecture/013-technology-stack.md`, then freeze versions for the first admin-app delivery slice.

### Full-stack currency rule

Phase 0 is not complete until the broader technology catalog in `specs/01-platform/architecture/013-technology-stack.md` has been re-verified and any repo-owned pins or images that participate in local development, validation, or admin-app delivery have been moved to the latest stable version within the selected product line, or to the latest version inside an intentionally selected pre-release channel where the catalog explicitly tracks one. That repo-wide verification has now been refreshed for planning purposes, and it already identifies required non-app upgrades including Bun `1.3.11 -> 1.3.13`, Turborepo `2.9.6 -> 2.9.12`, Convex `1.35.1 -> 1.38.0`, Keycloak `26.6.0 -> 26.6.1`, OpenTelemetry Collector `0.149.0 -> 0.151.0`, Prometheus `3.11.1-distroless -> 3.11.3-distroless`, Tempo `2.8.3 -> 2.10.5`, Grafana `12.4.2 -> 13.0.1`, Meilisearch `1.41.0 -> 1.43.0`, Postal `3.3.4 -> 3.3.6`, and Unleash Server `7.6.1 -> 7.6.3`. The table above is only the admin-app-facing subset of that gate.
...
The admin app is not waiting on a broad backend rewrite, but several concepts must be explicit so the frontend does not outrun the platform contracts.

| Concept                                                | Why the admin app needs it                                                                                                                 | Current state                                                                                                        | What must happen before or while UI lands                                                                                                 | Required validation and docs                                                                                               |
| ------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| Operations Home summary contract                       | The root route needs posture counts, queues, alerts, and recent activity without stitching raw domain payloads in the client.              | New app aggregation over validated services.                                                                         | Add an app-safe aggregated summary/helper and keep the composition out of route components.                                               | `tests/platform`, admin browser coverage for the home route, and Playwright coverage once stable.                          |
| Tenant Workspace aggregate projection                  | The tenant cockpit needs one tenant-scoped view across tenant management, billing, branding, audit, and support-safe data.                 | New app aggregation over validated services.                                                                         | Add a tenant-scoped projection/helper rather than composing many unrelated payloads in the route component.                               | `tests/platform`, backend-e2e where transport changes, and admin browser coverage for the tenant route.                    |
| Operator capability snapshot                           | Navigation, screen entry, and action visibility must follow real permissions and entitlements rather than hardcoded UI checks.             | Permission scopes and authorization are validated, but the plan now requires an app-safe capability surface.         | Add a backend-backed capability snapshot/helper keyed to the current trusted session and route/action vocabulary.                         | `tests/platform` for authorization mapping plus browser coverage for hidden/denied states.                                 |
| Access-control and projection-profile operator surface | The admin app is expected to manage permissions and projection profiles, which needs more than read-only authorization checks.             | Authorization is validated for current flows, but broader operator review/revocation workflows remain a tracker gap. | Add explicit tuple review/revocation and projection-profile management surfaces before the route ships.                                   | Access/domain tests, `tests/platform`, backend-e2e if transport changes, plus browser coverage when the route lands.       |
| Governed reason catalogs and action policy metadata    | Reveal dialogs, approval drawers, rejection flows, and high-risk actions need governed reason options and policy hints.                    | Reason capture exists in current slices, but a shared catalog/policy surface is not yet explicit in the plan.        | Backend owns the enumerations and policy metadata; the UI consumes returned values instead of inventing local dropdown choices.           | Contract/service tests, backend-e2e where flows cross transport boundaries, and browser coverage once the route uses them. |
| Server-driven list query envelopes                     | Dense screens need typed filters, sort, pagination, export, and row-detail lookup without oversharing data or relying on client filtering. | Mixed by surface.                                                                                                    | Standardize typed query/search/sort/pagination contracts as each screen lands.                                                            | `tests/platform`, backend-e2e for list/export transport, and browser URL-state/filter tests.                               |
| Support-safe break-glass detail projection             | The final support screen needs richer incident detail than the current core slice exposes.                                                 | Targeted contract expansion still needed.                                                                            | Expand the support-safe incident projection before shipping the richer detail pane that shows reviewer/expiry context.                    | Support-operations service tests, backend-e2e, and doc/tracker updates.                                                    |
...
| Optional step-up auth policy                           | Secret reveals or high-risk actions may require re-auth or MFA depending on security policy.                                               | Not assumed as a current backend guarantee.                                                                          | Only add if security requires it, but when added the challenge state and audit trail must be backend-owned.                               | Access/security tests first, then browser and Playwright coverage once the UI depends on it.                               |
...
The following app-layer helpers will likely be needed:
...
1. Refresh the admin-app-facing packages above and re-verify the full repo technology catalog in `013-technology-stack.md` so every participating stack version is at the latest stable release within its selected product line, or at the latest version inside an intentionally selected pre-release channel where the catalog explicitly tracks one.
