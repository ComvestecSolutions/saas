...
Use this runbook to start, initialize, verify, and perform the minimum post-start bootstrap for the optional `hardened` profile in `ops/docker/compose.yml`.
...
1. Start Kong and Vault through the `hardened` profile in `ops/docker/compose.yml`.
...
2. Start the hardened profile.

   ```bash
   docker compose --env-file .env -f ops/docker/compose.yml --profile hardened up -d
   ```

3. Confirm the profile is up.

   ```bash
   docker compose --env-file .env -f ops/docker/compose.yml --profile hardened ps
   ```
...
1. Check the current Vault seal state.
   ...
   ```bash
   docker compose --env-file .env -f ops/docker/compose.yml --profile hardened exec vault vault status
   ```

2. On a fresh `vault_data` volume, initialize Vault once and capture the output securely.

   ```bash
   docker compose --env-file .env -f ops/docker/compose.yml --profile hardened exec vault vault operator init
   ```

3. Unseal Vault with the required number of unseal keys from the initialization output.

   ```bash
   docker compose --env-file .env -f ops/docker/compose.yml --profile hardened exec vault vault operator unseal <key-1>
   docker compose --env-file .env -f ops/docker/compose.yml --profile hardened exec vault vault operator unseal <key-2>
   docker compose --env-file .env -f ops/docker/compose.yml --profile hardened exec vault vault operator unseal <key-3>
   ```

4. Log in with the root token from initialization.

   ```bash
   docker compose --env-file .env -f ops/docker/compose.yml --profile hardened exec vault vault login <root-token>
   ```

5. Enable a minimal KV v2 mount for foundation-managed secrets.

   ```bash
   docker compose --env-file .env -f ops/docker/compose.yml --profile hardened exec vault vault secrets enable -path=platform kv-v2
   ```

6. Verify that the mount is usable.

   ```bash
   docker compose --env-file .env -f ops/docker/compose.yml --profile hardened exec vault vault kv put platform/example service=test
   docker compose --env-file .env -f ops/docker/compose.yml --profile hardened exec vault vault kv get platform/example
   ```
...
4. After changing `ops/docker/security/kong.yml`, restart Kong to reload the declarative configuration.
   ...
   ```bash
   docker compose --env-file .env -f ops/docker/compose.yml --profile hardened up -d kong
   ```
