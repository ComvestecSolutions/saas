# Stitch Prompt: Identity Email Verification Email
...
**Template id:** `identity-session.email-verification`  
...
Design the **email verification email** for **Comvestec Solutions SaaS Foundation**, a governed multi-tenant SaaS platform.

This is a **security-sensitive transactional email** tied to the identity-session flow. The platform's accepted direction keeps identity handoffs aligned with tenant branding and branded redirects, even when the final verification screen is owned by an identity boundary. The email therefore needs to feel consistent with the platform's high-trust product system while staying simple, safe, and unmistakably legitimate.
...
- `public-web` should feel like an **editorial enterprise** front door in dark mode: spacious charcoal canvas, restrained verdigris or teal plus copper accents, and refined brand-forward hierarchy.
- `product-app` should feel like a **precision workspace** in dark mode: deep-neutral shell, elevated panels, crisp operational spacing, and focused CTA treatment.
- `admin-app` should feel like an **operations control tower** in dark mode: denser graphite panels, technical labels, and visibly governed seriousness.
- If the verification email is for an admin-app user or operator auth flow, it must clearly adopt the **admin-app** look and feel.
...
- The email's job is to help a user verify an email address securely, not to explain the whole auth system.

## Visual direction

- Create a **security-first transactional** aesthetic in the active app surface.
- Use a minimal, highly legible dark-mode layout with one dominant CTA and conservative accent use.
- The design should feel official, calm, and hard to mistake for a phishing-style fake.
- Prefer clear spacing, strong heading hierarchy, compact time-sensitivity treatment, and restrained brand presence.
- Avoid decorative illustrations, marketing banners, or busy dashboard motifs.

## What this email must accomplish

1. Confirm that email verification is required or was requested.
2. Provide one clear path to continue verification.
3. Communicate any expiration window or time sensitivity clearly.
4. Reassure the recipient about what to do if they did not expect this email.
...
- Verification URL
- Expiry timestamp or verification-window hint
...
1. **Header**
   ...
   - Security-oriented label such as verify email

2. **Primary action block**
   - Headline that verification is needed
   - Supporting line that references the account or recipient email
   - Strong verify-email CTA button
...
- Do not include passwords, MFA codes, or unnecessary secret material.
...
Create a high-trust email verification email that feels safe, brand-consistent, app-aware, and unmistakably transactional, with one clear CTA and excellent clarity across desktop and mobile email clients.
