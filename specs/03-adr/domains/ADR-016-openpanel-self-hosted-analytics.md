...
Prefer the self-hosted OpenPanel deployment model as the foundation default. Keep OpenPanel in the main repo-managed Docker Compose deployment surface in `ops/docker/compose.yml`, with its included definition and local runtime assets owned by `ops/docker/analytics/`, so operators can bring up the OpenPanel stack through the same in-repo deployment surface as the rest of the local platform.
...
3. Keeping OpenPanel in its own concern-owned Compose file preserves a reviewable ownership boundary for the analytics services while still keeping OpenPanel in the current required dependency footprint.
...
6. Reusing the repository's main Compose surface for the OpenPanel service group aligns analytics with the rest of the local operator workflow while keeping ownership and runtime assets explicit.
