...
/**
 * `/desk/notify/$id` — spec-canonical Notification Center v2 detail
 ...
 * convention shipped on `/desk/run/$id`, `/desk/incident/$incidentId`,
 * `/desk/legal-hold/$holdId`, `/desk/delivery/$deliveryId`, and
 * `/desk/api-key/$keyId`. Mounts as a child of the `/desk/notify`
 * list layout so the list spine stays visible above the detail
 * pane (mirrors the `/desk/flag` + `/desk/flag/$flagKey` pattern).
 *
...
export const Route = createAdminAppFileRoute("/desk/notify/$id")({
  loader: async ({ params }) => {
  ...
  return (
    ...
    >
      <ScreenHeader
        ...
        breadcrumbs={[
          { label: "Resources" },
          { label: "Notifications", href: "/desk/notify" },
