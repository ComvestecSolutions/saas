...
/**
 * `/r/run/$id` — spec-canonical Workflow Run Detail v2 surface
 ...
 * on `/r/api-key/$keyId`). The guards capture the operator's
 * reason + note today.
 *
...
export const Route = createAdminAppFileRoute("/r/run/$id")({
  loader: async ({ params }) => {
  ...
  return (
    ...
    >
      <ScreenHeader
        ...
        breadcrumbs={[
          { label: "Resources" },
          { label: "Workflow runs", href: "/r/runs" },
          { label: run.runId },
        ]}
        subtitle={
          <>
