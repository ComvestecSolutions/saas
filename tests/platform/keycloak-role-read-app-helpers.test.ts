...
import { getKeycloakRoleByIdFromEnvironment } from "@comvestec/platform";

const baseRequestContext: RequestContext = {
  actorType: actorType.platformOperator,
  actorId: "usr_kr_app_helpers_test",
  sessionId: "sess_kr_app_helpers_test",
  correlationId: "corr_kr_app_helpers_test",
  reason: "keycloak role read app helpers smoke",
  tenant: {
...
const expectRuntimeEnvironmentFailure = async (
...
describe("keycloak-role-read app helpers", () => {
  it("exposes the canonical *FromEnvironment helper as a function", () => {
    expect(typeof getKeycloakRoleByIdFromEnvironment).toBe("function");
    expect(getKeycloakRoleByIdFromEnvironment.length).toBe(2);
  });

  it("getKeycloakRoleByIdFromEnvironment decodes the operator environment at the boundary", async () => {
    await expectRuntimeEnvironmentFailure(
  ...
  it("re-uses the shared loadRuntimeModuleOrDie seam and shapes no Request/Response payloads", async () => {
    ...
    const source = await fs.readFile(
      path.resolve(
        "packages/platform/src/services/apps/keycloak-role-read-actions.ts",
      ),
      "utf8",
    );
