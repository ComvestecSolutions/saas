...
The live smoke kickoff requires a real `POLAR_ACCESS_TOKEN` and a `POLAR_WEBHOOK_SECRET` captured from Polar CLI local forwarding. With the backend API running, use `polar listen http://127.0.0.1:3010/api/subscriber-journey/billing/webhooks/polar`, write the reported secret into Vault, and then run `bun run backend:subscriber-journey:live-smoke:local`. The repo-tracked local Polar host defaults now stay on `https://sandbox-api.polar.sh/v1` for both `POLAR_API_URL` and `POLAR_API_BASE_URL`. If you intentionally point local tooling at another Polar host, override both keys together in `.env.local`; the local runtime will mirror a single non-default override onto the sibling key, but conflicting overrides still fail closed before admin or backend flows start. `SUBSCRIBER_JOURNEY_PUBLIC_BASE_URL` is now optional and only needed when you want the smoke script to print externally reachable webhook and replay URLs.
