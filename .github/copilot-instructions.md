...
2. Keep direct Node runtime usage only when an upstream platform explicitly requires it, such as Convex action files that must declare `"use node"` or type-only Node compatibility surfaces.
